...
describe("addAtomFromStub", () => {
  it("bonds the new atom to its parent symmetrically", () => {
    ...
    const grown = addAtomFromStub(seed, seed.rootId, 0, "O", 1);
    ...
    expect(child.angleFromParent).toBe(0);
  });

  it("never reuses an id, even after deletions", () => {
    let graph = createSeedGraph();
    graph = addAtomFromStub(graph, graph.rootId, 0, "C", 1); // id "1"
    graph = deleteAtomSubtree(graph, "1");
    graph = addAtomFromStub(graph, graph.rootId, 90, "C", 1); // must not reuse "1"
  ...
  it("throws when the parent doesn't exist", () => {
    const seed = createSeedGraph();
    expect(() => addAtomFromStub(seed, "missing", 0, "C", 1)).toThrow();
  });
});
...
describe("setBondOrder", () => {
  ...
  it("is not clamped by valence, so nitro's hypervalent N is buildable", () => {
    ...
    let graph = createSeedGraph();
    graph = addAtomFromStub(graph, graph.rootId, 0, "N", 1); // id "1"
    graph = addAtomFromStub(graph, "1", -45, "O", 1); // id "2"
    graph = addAtomFromStub(graph, "1", 45, "O", 1); // id "3"
...
describe("deleteAtomSubtree", () => {
  it("prunes everything reachable only through the deleted atom", () => {
    // A(seed) - B - C - D
    let graph = createSeedGraph();
    graph = addAtomFromStub(graph, graph.rootId, 0, "C", 1); // B = "1"
    graph = addAtomFromStub(graph, "1", 0, "C", 1); // C = "2"
    graph = addAtomFromStub(graph, "2", 0, "C", 1); // D = "3"
...
describe("closeRingBond", () => {
  it("bonds two existing atoms without creating a new one", () => {
    ...
    graph = addAtomFromStub(graph, "1", 0, "C", 1); // "2"
